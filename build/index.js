...
function formatVulnerability(vuln) {
    ...
    if (vuln.affected && vuln.affected.length > 0) {
        ...
        if (affected.ranges && affected.ranges.length > 0) {
            ...
            if (introduced)
                lines.push(`Introduced in: ${introduced}`);
            if (fixed)
                lines.push(`Fixed in: ${fixed}`);
        }
    }
...
server.tool("check_package_cves", "Check for known CVEs/vulnerabilities in npm packages before installation", {
    packageName: z
        .string()
    ...
    version: z
        .string()
        .optional()
        .describe("Specific version to check (optional). If not provided, checks all known vulnerabilities for the package."),
...
server.tool("check_packages_bulk_cves", "Check multiple npm packages for CVEs at once (useful before installing dependencies)", {
    packages: z
        .array(z.object({
    ...
    try {
        const results = await Promise.all(packages.map(async (pkg) => {
            try {
                const result = await checkPackageVulnerabilities(pkg.name, pkg.version);
                return {
                    package: pkg.name,
                    version: pkg.version,
